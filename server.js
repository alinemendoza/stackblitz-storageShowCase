/**
 * Web Storage Explorer - Server
 * 
 * This is the main server file for the Web Storage Explorer application.
 * It uses Express.js with Handlebars templates to create a web application
 * that demonstrates different storage methods for web development.
 * 
 * The application demonstrates:
 * 1. JSON file storage (server-side)
 * 2. Session storage (server-side)
 * 3. localStorage (client-side, browser-based)
 * 4. Cookies (client-side, sent with HTTP requests)
 * 5. API Integration (accessing and caching external data)
 */
...
import { quizQuestions, getFeedback } from './server/data/quiz-questions.js';

// Get current directory name (needed with ES modules)
...
app.get('/cookies', (req, res) => {
    ...
    for (const [name, value] of Object.entries(req.cookies)) {
        // Skip session cookie to avoid confusion
...
app.post('/cookies/set', (req, res) => {
    ...
    // Basic validation
    if (!name || !value) {
        req.session.message = 'Name and value are required';
        req.session.messageType = 'error';
        return res.redirect('/cookies');
    }
    
    // Set expiration date
    const days = parseInt(expiration) || 7;
    ...
    // Flash message and redirect
    req.session.message = `Cookie "${name}" set successfully!`;
    req.session.messageType = 'success';
    res.redirect('/cookies');
});

// Delete a cookie
app.post('/cookies/delete/:name', (req, res) => {
    const name = req.params.name;
    
    // Delete by setting past expiration date
    res.cookie(name, '', {
        expires: new Date(0),
        httpOnly: true
    });
    ...
    // Flash message and redirect
    req.session.message = `Cookie "${name}" deleted successfully!`;
    req.session.messageType = 'success';
    res.redirect('/cookies');
});
